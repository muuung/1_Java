...
public class AbstractionService {
	...
	public void ex1() {
		...
		// 마법의 소라고둥님 추상 클래스는 정말 객체 생성이 안되나요? 안돼.
		// Animal a = new Animal(); Cannot instantiate the type Animal
		
		// Animal을 상속 받은 자식 클래스를 이용해 객체 생성
		Fish f1 = new Fish();
		f1.setType("물고기"); // 자식 객체 내부에 부모 부분이 있음을 확인
		// f1.breath();
		
		Person p1 = new Person();
		p1.setType("지구파괴범"); // 자식 객체 내부에 부모 부분이 있음을 확인
		
		// 추상 클래스는 부모 타입의 참조 변수로 사용할 수 있음
		// 다형성의 업캐스팅
		Animal a1 = new Fish();
		...
		// 다들 강제 오버라이딩한 breath()라는 공통적은 메서드를
